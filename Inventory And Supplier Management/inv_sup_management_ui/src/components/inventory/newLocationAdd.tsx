...
export default function NewLocationPopUp({className}:props){
  const isOpen:boolean = useSelector((state:RootState)=>state.inventory.AddNewLocPopUpVisible)
  const dispatch:any = useDispatch();
  useEffect(()=>{
    document.body.classList.toggle("overflow-hidden",isOpen)
  },[isOpen])
  return (
    <div className={`w-full h-full top-0 fixed z-1000 backdrop-blur-[2px] flex items-center justify-center p-2 ${className} ${isOpen?"":"hidden"}`}>
    {/*Parent container*/}
...
<p className={`text-lg font-semibold flex flex-row items-center justify-center gap-2`}><MapPlus/>Add New Location</p>
        <X size={30} onClick={()=>dispatch(setAddNewLocPopUpVisible(false))} className={"p-1 rounded hover:bg-grayscale/10"}/>
        </div>
